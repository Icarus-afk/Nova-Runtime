...
impl Command {
    pub fn execute(&self, ctx: &CommandContext) -> anyhow::Result<()> {
        match self {
            Command::Runtime(cmd) => cmd.execute(ctx),
            Command::Config(cmd) => cmd.execute(ctx),
            Command::Auth(cmd) => cmd.execute(ctx),
            Command::Queue(cmd) => cmd.execute(ctx),
            Command::Scheduler(cmd) => cmd.execute(ctx),
            Command::Search(cmd) => cmd.execute(ctx),
            Command::Blob(cmd) => cmd.execute(ctx),
            Command::Sql(cmd) => cmd.execute(ctx),
            Command::Db(cmd) => cmd.execute(ctx),
            Command::Cache(cmd) => cmd.execute(ctx),
            ...
            Command::Run { config, data_dir } => {
                ...
                println!("  Use `novad` binary for the full daemon.");
                Ok(())
            }
        }
    }
