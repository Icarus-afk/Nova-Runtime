...
impl EndpointDef {
    ...
    fn post(path: &'static str, body: Option<&'static str>) -> Self {
        Self {
            path,
            method: Method::POST,
            body,
...
impl HttpSubsystem {
    pub fn new(target: &str) -> Self {
        ...
        let endpoints = vec![
            ...
            EndpointDef::post(
                "/api/v1/sql/query",
                Some(r#"{"query":"SELECT * FROM iot_weather_sensors"}"#),
            ),
        ];
        Self {
            client,
...
impl Subsystem for HttpSubsystem {
        ...
        match self.client.get(&format!("{}/health", self.target)).send() {
            ...
            Err(_) => {
                ...
                log!(
                    ...
                    LogLevel::Warn,
                    "http",
                    format!("{} unreachable — requests suspended", self.target)
                );
            }
        }
    }
    fn tick(&mut self, ctx: &mut TickContext) {
        ...
        let count = if self.server_down {
            1
        } else {
            ctx.rng.range(1, (ctx.load as u64 / 8 + 2).min(8))
        };
        ctx.metrics
            .requests_total
        ...
        for _ in 0..count {
            ...
            let result = match self.build_request(ep).send() {
                ...
                Err(e) => {
                    ...
                    log_detail!(
                        ...
                        LogLevel::Error,
                        "http",
                        format!("{method_str} {path} → {kind} ({dur_ms:.0}ms)"),
                        Some(req_id),
                        Some(dur_ms as u64)
                    );
                    continue;
            ...
            if status >= 500 {
                ...
                log_detail!(
                    ...
                    LogLevel::Error,
                    "http",
                    format!("{method_str} {path} → {status} ({dur_ms}ms)"),
                    Some(req_id),
                    Some(dur_ms)
                );
            } else if status >= 400 {
...
impl AuthSubsystem {
    pub fn new() -> Self {
        Self {
            users: &["admin", "alice", "bob", "charlie", "dave", "eve"],
        }
    }
}
        ...
        for _ in 0..count {
            ...
            if ok {
                ...
                log!(
                    ...
                    LogLevel::Info,
                    "auth",
                    format!("User '{user}' authenticated")
                );

                let event = nova_event::EventBuilder::new("auth.user.login").map(|b| {
                    b.source(nova_event::Subsystem::System, "auth", "sim", "nova-sim")
                        .build(
                            serde_json::to_vec(&serde_json::json!({"user": user}))
                                .unwrap_or_default(),
                        )
                });
                if let Ok(e) = event {
                    let _ = ctx.event_bus.publish(e);
                }
            } else {
        ...
        for _ in 0..count {
            ...
            if is_error {
                ...
                log!(
                    ...
                    LogLevel::Error,
                    "sql",
                    format!("Query failed: {query} ({dur}ms) — connection timeout")
                );
            } else if slow {
                ctx.metrics.sql_slow.fetch_add(1, Ordering::Relaxed);
                log!(
                    ...
                    LogLevel::Warn,
                    "sql",
                    format!("Slow query ({dur}ms): {query}")
                );
            } else if ctx.verbose {
                log!(ctx, LogLevel::Info, "sql", format!("{query} ({dur}ms)"));
            }

            let ev_type = if query.starts_with("INSERT")
            ...
            let event =
                nova_event::EventBuilder::new(&format!("sql.{ev_type}.executed")).map(|b| {
                    b.build(
                        serde_json::to_vec(
                            &serde_json::json!({"query": query, "duration_ms": dur}),
                        )
                        .unwrap_or_default(),
                    )
                });
    ...
    fn maybe_evict(&mut self, ctx: &mut TickContext) {
        while self.store.len() > self.max_entries {
            ...
            log!(
                ...
                LogLevel::Warn,
                "cache",
                format!("Evicted key '{key}' — capacity reached")
            );
        }
    }
}
    ...
    fn tick(&mut self, ctx: &mut TickContext) {
        ...
        for _ in 0..ops {
            ...
            if hit {
                ...
                if ctx.verbose {
                    log!(ctx, LogLevel::Info, "cache", "Cache HIT".into());
                }
            } else {
                ...
                log!(
                    ...
                    LogLevel::Info,
                    "cache",
                    format!("Cache MISS — populated key '{key}' from origin")
                );
        ...
        if topic.starts_with("cache.invalidate") {
            ...
            if !key.is_empty() && !key.starts_with("pattern.") {
                ...
                log!(
                    ...
                    LogLevel::Info,
                    "cache",
                    format!("Invalidated key '{key}' via event bus")
                );
                ...
                log!(
                    ...
                    LogLevel::Info,
                    "cache",
                    format!("Invalidated pattern '{pattern}*' via event bus")
                );
            }
        }
    }
...
impl Subsystem for QueueSubsystem {
    ...
    fn tick(&mut self, ctx: &mut TickContext) {
            ...
            if self.messages.len() < self.max_depth as usize {
                ...
                if ctx.verbose {
                    log!(
                        ...
                        LogLevel::Info,
                        "queue",
                        format!("Published '{msg}' to queue 'default'")
                    );
                }
            }
        }
        ...
        for _ in 0..consume.min(self.messages.len()) {
            ...
            if ok {
                ...
                log!(
                    ...
                    LogLevel::Error,
                    "queue",
                    format!("Consumer failed processing '{msg}' — requeueing")
                );
        ...
        ctx.metrics
            .queue_depth
            .store(self.messages.len() as u32, Ordering::Relaxed);
    }
    fn shutdown(&mut self, _ctx: &mut TickContext) {}
}
    ...
    fn tick(&mut self, ctx: &mut TickContext) {
        ...
        for _ in 0..jobs_fire {
            ...
            log!(
                ...
                LogLevel::Info,
                "sched",
                format!("Fired scheduled job '{job}'")
            );

            let event = nova_event::EventBuilder::new("scheduler.job.fired").map(|b| {
                b.build(serde_json::to_vec(&serde_json::json!({"job": job})).unwrap_or_default())
            });
            if let Ok(e) = event {
                let _ = ctx.event_bus.publish(e);
            }
        }
        let completed = ctx.rng.range(
            0,
            ctx.metrics.scheduler_jobs_active.load(Ordering::Relaxed) as u64 + 1,
        ) as u32;
        ctx.metrics
            .scheduler_jobs_active
            .fetch_sub(completed, Ordering::Relaxed);
    }
    fn shutdown(&mut self, _ctx: &mut TickContext) {}
}
...
impl Subsystem for SearchSubsystem {
    ...
    fn tick(&mut self, ctx: &mut TickContext) {
        ...
        for _ in 0..indexing {
            ...
            if ctx.verbose {
                log!(
                    ...
                    LogLevel::Info,
                    "search",
                    "Indexed document in 'main' index".into()
                );
            }
        }
        ...
        for _ in 0..queries {
            ...
            let results = if self.index_size > 0 {
                ctx.rng.range(0, self.index_size as u64)
            } else {
                0
            };
            if ctx.verbose {
                log!(
                    ...
                    LogLevel::Info,
                    "search",
                    format!("Search query returned {results} results ({dur}ms)")
                );
            }
        }
    }
    ...
    fn tick(&mut self, ctx: &mut TickContext) {
        ...
        for _ in 0..reads {
            if ctx.failure_injected && ctx.rng.bool(0.08) {
                ...
                log!(
                    ...
                    LogLevel::Warn,
                    "storage",
                    "Read retry #2 — I/O timeout on page 0x3A1F".into()
                );
            }
        }
        for _ in 0..writes {
            if ctx.failure_injected && ctx.rng.bool(0.06) {
                ...
                log!(
                    ...
                    LogLevel::Warn,
                    "storage",
                    "Write retry #1 — checksum mismatch, replaying WAL".into()
                );
            }
        }
        if ctx.verbose && reads + writes > 0 {
            log!(
                ...
                LogLevel::Info,
                "storage",
                format!("{reads} reads, {writes} writes completed")
            );
        }
    }
    fn shutdown(&mut self, _ctx: &mut TickContext) {}
...
impl Subsystem for WorkerSubsystem {
    ...
    fn tick(&mut self, ctx: &mut TickContext) {
        ...
        ctx.metrics
            .workers_idle
            .store(total - active, Ordering::Relaxed);
    }
    fn shutdown(&mut self, _ctx: &mut TickContext) {}
}
...
impl Subsystem for EventBusSubsystem {
    ...
    fn tick(&mut self, ctx: &mut TickContext) {
        ...
        for _ in 0..count {
            ...
            let event = nova_event::EventBuilder::new(topic).map(|b| {
                b.source(nova_event::Subsystem::System, "sim", "nova-sim", "0")
                    .build(vec![])
            });
            if let Ok(e) = event {
                let _ = ctx.event_bus.publish(e);
