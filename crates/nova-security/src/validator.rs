...
impl InputValidator {
    ...
    pub fn validate_utf8(&self, data: &[u8]) -> Result<()> {
        match std::str::from_utf8(data) {
            Ok(_) => Ok(()),
            Err(e) => Err(SecurityError::Validation(format!("Invalid UTF-8: {}", e))),
        }
    }
    ...
    pub fn validate_headers(
        ...
        for (i, (name, value)) in headers.iter().enumerate() {
            if name.len() > 128 {
                errors.push(format!(
                    "Header name at index {} exceeds max length of 128",
                    i
                ));
            }
            if name.contains('\0') {
            ...
            if value.len() > 4096 {
                errors.push(format!(
                    "Header value at index {} exceeds max length of 4096",
                    i
                ));
            }
            if value.contains('\0') {
    ...
    pub fn validate_query_params(
        ...
        for (i, (name, value)) in params.iter().enumerate() {
            if name.len() > 128 {
                errors.push(format!(
                    "Query param name at index {} exceeds max length of 128",
                    i
                ));
            }
            if name.contains('\0') {
                errors.push(format!(
                    "Query param name at index {} contains null bytes",
                    i
                ));
            }
            if value.len() > 4096 {
                errors.push(format!(
                    "Query param value at index {} exceeds max length of 4096",
                    i
                ));
            }
            if value.contains('\0') {
                errors.push(format!(
                    "Query param value at index {} contains null bytes",
                    i
                ));
            }
        }
    ...
    pub fn sanitize_path(&self, path: &str) -> Result<String> {
        if path.contains('\0') {
            return Err(SecurityError::Validation("Path contains null bytes".into()));
        }
        ...
        for component in components {
            match component {
                ...
                ".." => {
                    if result.is_empty() {
                        return Err(SecurityError::Validation("Path traversal detected".into()));
                    }
                    result.pop();
                }
                _ => {
...
mod tests {
    ...
    fn test_validate_query_params() {
        ...
        let err = validator
            .validate_query_params(&invalid_params)
            .unwrap_err();
        assert!(err[0].contains("exceeds max length"));
    ...
    fn test_sanitize_path() {
        ...
        // Happy path
        assert_eq!(
            validator.sanitize_path("/valid/path").unwrap(),
            "/valid/path"
        );
    ...
    fn test_sanitize_path_component() {
        // Happy path
        assert_eq!(
            sanitize_path_component("valid_component").unwrap(),
            "valid_component"
        );
