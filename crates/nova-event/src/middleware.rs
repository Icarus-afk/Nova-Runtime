use crate::{Event, EventError, EventId, SubscriberId};
use std::sync::Arc;
...
mod tests {
    ...
    impl EventMiddleware for TestMiddleware {
        fn before_publish(&self, _event: &mut Event) -> Result<MiddlewareAction, EventError> {
            ...
            if self.reject {
                Ok(MiddlewareAction::Reject(EventError::Internal(
                    "rejected".into(),
                )))
            } else {
    ...
    fn test_middleware_before_publish_continue() {
        ...
        let mut event = crate::EventBuilder::new("test.event")
            .unwrap()
            .build(vec![]);
    ...
    fn test_middleware_after_publish() {
        let mw = TestMiddleware::new();
        let event = crate::EventBuilder::new("test.event")
            .unwrap()
            .build(vec![]);
        let id = EventId::new();
